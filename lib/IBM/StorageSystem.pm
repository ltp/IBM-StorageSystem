...
our @ATTR = qw(auth_service_cert_set auth_service_configured 
...
time_zone total_allocated_extent_capacity total_free_space 
total_mdisk_capacity total_overallocation total_used_capacity 
total_vdisk_capacity total_vdiskcopy_capacity);

our @STAT = qw(compression_cpu_pc cpu_pc drive_r_io drive_r_mb drive_r_ms 
drive_w_io drive_w_mb drive_w_ms fc_io fc_mb iscsi_io iscsi_mb mdisk_r_io 
mdisk_r_mb mdisk_r_ms mdisk_w_io mdisk_w_mb mdisk_w_ms sas_io sas_mb 
total_cache_pc vdisk_r_io vdisk_r_mb vdisk_r_ms vdisk_w_io vdisk_w_mb 
vdisk_w_ms write_cache_pc);
...
foreach my $stat ( keys %{ $STATS } ) { 
        ...
        *{ __PACKAGE__ .'::'. $stat } = 
        sub {
                my( $self, $t ) = @_; 
                $t ||= 'minute';
                my $stats = $self->__lsperfdata( 
					cmd   => "$STATS->{$stat}->{cmd} -t $t",
...
#
# Each hash represents an object type that we will generate methods for - for 
# example, given the below;
#
# 	drive => 
...
#	},
#
# drive	- 	is the object type - in this case a drive in Storwize 
#		nomenclature representing a physical hard drive.
#
# bcmd	-	is the base cmd to be executed in the Storwize CLI to retrieve 
#		a list of drive objects.  This parameter is only required for 
#		object types where object enumeration is required as a 
#		prequisite to execution of object specific commands.
#
#		Using the example above for 'lsdrive -nohdr -delim :', the 
#		base cmd is necessary to first enumerate all drives to obtain 
#		a summary listing of all drives and their ID's.  The ID's can 
#		then be used in	the 'cmd' command (e.g. lsdrive 1) to obtain 
#		detailed information on each drive.  
#
#		As there is no way in which to obtain detailed information of 
#		some objects without specifying the object ID, the bcmd is 
#		necessary to first produce a list of these ID's.  Contrast the 
#		operation of the lsdrive command and the need for an ID 
#		parameter to obtain detailed information with the operation of 
#		the lsnode command, which provides detailed information on all 
#		nodes without any parameters.
#
#		*NOTE* that the bcmd is	not necessary for all objects - only 
#		those which there is no single command to produce a detailed 
#		listing without specifying an ID type parameter.  Note also, 
#		that you probably want to specify additional options to the 
#		bcmd command (like -nohdr and -delim :) to prevent header
#		information from being parsed.
#
# cmd	-	is the CLI command that will be used to retrieve information 
#		about the object.  *NOTE* the above information	on the use of 
#		the bcmd value and how this command works in conjunction with 
#		it.
#
# id	-	is a single field in the CLI command output (or multiple fields 
#		concatenated by colon) that is able to uniquely identify each 
#		object in a global context for this object type.  For example; 
#		in a systems with more than one enclosure it is necessary to 
#		identify an enclosure PSU by specifying both the enclosure id 
#		and the PSU id - e.g. Enclosure 2, PSU 1 - which when separated 
#		by a colon would become 2:1 which is sufficient to uniquely 
#		identify a enclsoure PSU in a system of any number of 
#		enclosures.
#
# class	-	the Perl package namespace into which this object will be 
#		blessed as a class.
#
# type	-	the 'type' of object - this is usually the same as the root key
#		type, however it must also be globally unique within this class.
#		This value is used internally to implement object caching and 
#		is not used in method naming so need not necessarily intuitive 
#		to a user.
#
# sl	-	This optional parameter is used to designate the CLI command 
#		output type as 'single-line' - this is in contrast to 
#		'multi-line' output.  In general, CLI command output falls into
#		two categories; 'single-line' output contains output on multiple
#		objects with each line representative of a unique object - this 
#		may also be referred to as 'row-based' data.  An example of 
#		such output is the 'lsnode' command.
#
#		'multi-line' output contains output about a single object as 
#		key-value pairs over multiple lines, may also be referred to as
#		'columnar' data.  An output of such output is the 'lsdrive' 
#		command when executed with a valid drive id parameter 
#		(e.g. lsdrive 1).
#
#		By default, CLI commands are assumed to use columnar output, so
#		it is necessary for any commands using row-based output to also
#		specify a true value for the 'sl' key to ensure that the output 
#		is parsed correctly.  CLI commands that output columnar data 
#		should not specify this command.
...
sub new {
	...
	$args{user}
		? $self->{user}	= $args{user}
		: croak 'Mandatory parameter "user" not given';

	$args{host}
		? $self->{host}	= $args{host}
		: croak 'Mandatory parameter "host" not given';

	$args{key_path}
		? $self->{key_path} = $args{key_path}
		: croak 'Mandatory parameter "key_path" not given';
	...
	$self->{ssh}->error and croak 'Could not create Net::OpenSSH object: ' 
					. $self->{ssh}->error . "\n";

	unless ( $args{no_stats} ) {
		$self->__lssystem;
		$self->refresh_system_stats;
		$self->{stats_threshold} = ( $args{stats_threshold} 
						? $args{stats_threshold} 
						: 0 
					);
	}
...
sub refresh_system_stats {
	...
	foreach my $stat ( splice @{ 
				[ split /\n/, $self->__cmd( 'lssystemstats -gui -delim :' ) ]
				}, 1 
			) {

		my ( $name, $epoch, $current, $peak, $peak_time, $peak_epoch ) 
			= split /:/, $stat;

		$self->{$name} = IBM::StorageSystem::Statistic->new(
					$self, 
					name	=> $name, 
					epoch	=> $epoch, 
					current => $current,
...
#	Value_1 , Value_2,  Value_3,  ... Value_N
#
# To parse this data into an array of objects, we treat each row as a single 
# object. We split the column headers on a delimeter (usually :) and use each 
# header as a hash key for the corresponding value in each row - i.e.
#
#	Column_1 => Value_1,
#	Column_2 => Value_2 ...etc.
#
# Each hash is then passed to the constructor for this object type (given as 
# the 'class' argument in our %args hash), and the resultant object 
# conditionally cached within our IBM::StorageSystem object and pushed onto 
# our returned array.
#
# Note that this method includes a slight hack to cater for the lack of 
# non-unique per-object keys in CLI output for some object types.  In this 
# scenario it is necessary to create a unique key via composite fields 
...
sub __get_sl_objects {
	...
	foreach my $object ( @objs ) {
		...
		$self->{ $args{ type } }->{ $a{ $args{ id } } } 
			= $obj unless $args{ nocache };
		push @res, $obj
	}
...
# and treat each row as a name,value hash pair for an attribute of a single 
# object.  For example, the above example output would become:
#
#	Column_1 => Value_1,
#	Column_2 => Value_2,
#	...
...
# specified by the $args{ class } value and the resultant object is 
# optionally cached in our and pushed onto the return array.
#
# Things to note in this sub include the hack required to handle non-unique 
# column names. For example; an 'lsfabric' output may include multiple 'WWPN' 
# columns, so it is neccesary to identify duplicate column names and treat the 
# corresponding hash values as anonymous arrays rather than scalars.
...
done explicitly via the B<refresh> method of an L<IBM:StorageSystem::Statistic> 
object.
...
True if the auth_service_type is configured and either one of the following is 
true:
...
B<Note> that this method returns an array of tier capacity values, the index 
...
Returns an L<IBM::StorageSystem::Statistic> object for the average number of 
megabytes transferred per second for read operations to drives during the 
sample period.

=head3 drive_r_ms

Returns an L<IBM::StorageSystem::Statistic> object for the average amount of 
...
Returns an L<IBM::StorageSystem::Statistic> object for the total input/output 
...
Returns an L<IBM::StorageSystem::Statistic> object for the total number of 
...
Returns an L<IBM::StorageSystem::Statistic> object for the total I/O 
...
Returns an L<IBM::StorageSystem::Statistic> object for the total percentage 
...
Returns an L<IBM::StorageSystem::Statistic> object for the percentage of the 
...
B<Note> that you can call B<refresh> on individual system statistics which may 
...
unless explicitly done so by calling the B<refresh> method of the statistic 
...
Returns either a single, or list, of L<IBM::StorageSystem::StatisticsSet> 
objects containing L<IBM::StorageSystem::Statistics::ClusterThroughput> 
...
Returns either a single, or list, of L<IBM::StorageSystem::StatisticsSet> 
objects containing L<IBM::StorageSystem::Statistics::ClusterClientThroughput> 
...
Returns either a single, or list, of L<IBM::StorageSystem::StatisticsSet> 
objects containing L<IBM::StorageSystem::Statistics::ClusterCreateDeleteLatency> 
...
Returns either a single, or list, of L<IBM::StorageSystem::StatisticsSet> 
objects containing L<IBM::StorageSystem::Statistics::ClusterCreateDeleteOperations> 
...
Returns either a single, or list, of L<IBM::StorageSystem::StatisticsSet> 
objects containing L<IBM::StorageSystem::Statistics::ClusterOpenCloseLatency> 
...
Returns either a single, or list, of L<IBM::StorageSystem::StatisticsSet> 
objects containing L<IBM::StorageSystem::Statistics::ClusterReadWriteLatency> 
...
Returns either a single, or list, of L<IBM::StorageSystem::StatisticsSet> 
objects containing L<IBM::StorageSystem::Statistics::ClusterReadWriteOperations> 
...
Returns an L<IBM::StorageSystem::Array> object representing the array 
...
L<IBM::StorageSystem::Array> object.

B<Note> that this method is non-caching and the array information will always 
...
Returns an array of L<IBM::StorageSystem::Array> objects representing all 
...
Returns an array of L<IBM::StorageSystem::Disk> objects representing all disks 
...
L<IBM::StorageSystem::Drive> object.

B<note> that this method implements caching and that a cached object will be 
...
as an L<IBM::StorageSystem::Enclosure> object.

B<Note> that this is a caching method and that a cached object will be 
returned if one is present.  If you require a non-cached result, then please 
use the B<get_enclosure> method.
...
Returns an array of L<IBM::StorageSystem::Enclosure> objects representing all 
...
L<IBM::StorageSystem::Export> objects.
...
L<IBM::StorageSystem::Fabric> objects.
...
L<IBM::StorageSystem::FileSystem> object.
...
Returns an array of L<IBM::StorageSystem::FileSystem> objects representing all 
...
Returns an array of L<IBM::StorageSystem::Health> objects representative of 
all health sensors on the target system.

B<Note> that this method is only implemented on StorageSystem Unified systems 
...
L<IBM::StorageSystem::Host> object.

B<Note> that this is a caching method and a cached object will be returned if 
one exists.  If you require a non-cached object, then please use the 
B<get_host> method.

=head3 get_host( $hostname )

This is a functionally equivalent non-caching implementation of the B<host> 
method.
...
Returns an array of L<IBM::StorageSystem::Host> objects representing all host 
...
an L<IBM::StorageSystem::IOGroup> object.

B<Note> that this method implements caching and a cached object will be 
returned should one be present.  If you require a non-cached object then 
please use the B<get_iogroup> method.
...
Returns an array of L<IBM::StorageSystem::IOGroup> objects representing all 
...
L<IBM::StorageSystem::Interface> object.
...
Returns an array of L<IBM::StorageSystem::Interface> objects representing all 
...
L<IBM::StorageSystem::Mount> object.
...
This method returns an array of L<IBM::StorageSystem::Mount> objects 
...
L<IBM::StorageSystem::Node> object.

B<Note> that this method implements caching and that a cached object will be 
returned if one is available.  If you require a non-cached object, then please 
use the non-caching B<get_node> method.

=head3 get_node( $node )

This is a functionally equivalent non-caching implementation of the B<node> 
method.
...
Returns an array of L<IBM::StorageSystem::Node> objects representing all 
configured nodes on the target system.

=head3 pool( $pool )

Returns the pool identified by the value of the node parameter as a 
L<IBM::StorageSystem::Pool> object.

B<Note> that this method implements caching and that a cached object will be 
returned if one is available.  If you require a non-cached object, then please 
use the non-caching B<get_pool> method.

=head3 get_pool( $pool )

This is a functionally equivalent non-caching implementation of the B<pool> 
method.

=head3 get_pools( $pool )

Returns an array of L<IBM::StorageSystem::Pool> objects representing all 
configured pools on the target system.

=head3 replication( $eventlog_id )

Returns the replication event identified by the eventlog_id parameter as an 
L<IBM::StorageSystem::Replication> object.
...
L<IBM::StorageSystem::Replication> objects.
...
Returns a L<IBM::StorageSystem::Service> object representing the service 
...
This is a functionally equivalent non-caching implementation of the B<service> 
method.

=head3 get_services

Returns an array of L<IBM::StorageSystem::Service> objects representing all 
...
L<IBM::StorageSystem::Task> object.
...
This is a functionally equivalent non-caching implementation of the B<task> 
method.
...
Returns an array of L<IBM::StorageSystem::Task> objects representing all tasks 
...
Returns an L<IBM::StorageSystem::VDisk> object representing the VDisk 
identified by the numerical ID parameter.

B<Note> that this method implements caching to improve performance and reduce 
network overhead, and that a cached object will be returned if one is present.
If you require a non-cached object then please use the B<get_vdisk> method.

=head3 get_vdisk( $id )

This is a functionally equivalent non-caching implementation of the B<vdisk> 
method.
...
L<IBM::StorageSystem::VDisk> objects.
...
L<IBM::StorageSystem::Quota> objects.
...
C<bug-ibm-StorageSystem at rt.cpan.org>, or through the web interface at 
L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=IBM-StorageSystem>.  I will 
